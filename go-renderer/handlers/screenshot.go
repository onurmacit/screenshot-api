...
func (h *ScreenshotHandler) CaptureScreenshot(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		// Check for selector-specific errors
		if selectorErr := services.GetSelectorError(err); selectorErr != nil {
			errorCode = "selector_error"
			message = selectorErr.UserFriendlyMessage()
			hint = selectorErr.Hint()

			return c.Status(status).JSON(fiber.Map{
				"error":    errorCode,
				"message":  message,
				"selector": selectorErr.Selector,
				"operation": selectorErr.Operation,
				"hint":     hint,
				"details":  selectorErr.Error(),
			})
		}
